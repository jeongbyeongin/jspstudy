...
public class MemberAddService implements IMemberService {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 회원 추가를 실패할 경우 ajax() 메소드로 실패 메시지를 전달 하기 위해서 try - catch를 사용 할 것이다.
		/*
			try {
			
				응답 -> ajax()의 success로 넘어간다.
			
			} catch(Exception e){
			
				응답 -> ajax()의 error로 넘어간다.
		...
		*/
		...
		try {
			...
			*/
			...
			// 응답 코드 만들기
			response.setStatus(500); // 이 메시지들도 index의 에러 jqXHR로 간다.
			
			// 응답 데이터 만들기
			String msg = "신규 등록이 실패했습니다.\n입력 데이터를 확인하세요.";
			
			// 응답 실행 (catch문의 응답이므로 ajax의 error로 전달된다.)
			PrintWriter out = response.getWriter();
			out.println(msg);
			out.flush();
			out.close();
