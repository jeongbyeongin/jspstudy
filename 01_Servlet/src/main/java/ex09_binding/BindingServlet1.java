...
public class BindingServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
			...
			2. binding
				...
				3) 3개 영역
					...
					(2) HttpSession			: 웹 브라우저 종료 전까지 값을 저장할 수 있다.
					(3) ServletContext		: 컨텍스트(프로젝트, 애플리케이션)
				4) 속성(Attribute) 관련 메소드
					(1) getAttribute('속성')		: 값 가져오기(캐스팅이 필요할 수 있다.)
					(2) setAttribute('속성', 값)	: 값 저장하기(Object 타입으로 저장한다.)		어떤 속성에 어떤값을 저장하겠다.
		...
		*/
		...
		// ServletContext에 저장하기		// ServletContext ctx = getServletContext(); 이렇게도 부를 수 있다
		ServletContext ctx = getServletContext();
		ctx.setAttribute("a", 300);
		
		// 페이지 이동하기
		
		// 1.  HttpServletRequest의 전달이 없는 이동 : redirect, <a href="">, location.href=""  한통속이다
		// response.sendRedirect("/01_Servlet/BindingServlet2");
				
		// 2. HttpServleetRequest의 전달이 있는 이동 : forward
