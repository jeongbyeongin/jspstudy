...
public class HelloServlet extends HttpServlet {
	...
	*/
	...
	/*
		4. doGet
			1) GET 방식의 요청을 처리하는 메소드이다.		// a태그의 링크와 location = 주소를 바꿔주는 것
			2) GET 방식의 요청의 예시	// 메소드를 생략하면 GET방식이다. 동일한 프로젝트 안에서는 프로토콜부터 프로젝트이름까지 생략가능
				(1) <a href="http://localhost:9090/01_Servlet/HelloServlet">			HTML
				(2) <form action="http://localhost:9090/01_Servlet/HelloServlet">		HTML
				(3) location.href='http://localhost:9090/01_Servlet/HelloServlet'		JavaScript 	// location.href 다른페이지로 이동할 때 사용
				(4) open('http://localhost:9090/01_Servlet/HelloServlet', '', '')		JavaScript	// open 새창을 연다
				(5) $.ajax({															JavaScript
						type: 'GET'
	...
	*/
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//((( client <- server(Servlet) 보내주는 걸 response // 반대는 request // response.getWriter(자바 출력스트림) = printWriter )))// 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// POST 메소드를 통해서 넘어온 정보를 모두 doGet() 메소드에 넘긴다. POST -> doPost -> doGet / GET -> doGet으로 바로 넘어간다.
