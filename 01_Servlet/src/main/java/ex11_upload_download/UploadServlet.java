...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		...
		String lastModified = new SimpleDateFormat("yyyy-MM-dd").format(file.lastModified());	// file.lastModified() = 타입 스탬프 값을 전달
		String size = new DecimalFormat("#,##0").format(file.length() / 1024 + (file.length() % 1024 != 0  ? 0 : 1));	// 숫자 3자리 , 자동으로 붙여주는 메소드
